 ...
 * @file       bsp_imu.c
 * @brief      mpu6500 module driver, configure MPU6500 and Read the Accelerator
...
HAL_StatusTypeDef sendStatue1ViaCAN(CAN_HandleTypeDef *hcan, uint8_t FreValue[4]) {
    ...
    pHeader.DLC = 4;
    uint32_t pTxMailbox;
    HAL_StatusTypeDef status = HAL_CAN_AddTxMessage(hcan, &pHeader, FreValue, &pTxMailbox);
    return status;
}

HAL_StatusTypeDef sendStatue2ViaCAN(CAN_HandleTypeDef *hcan, uint8_t FreValue[4]) {
    CAN_TxHeaderTypeDef pHeader;
    pHeader.StdId = 0x402;
    pHeader.ExtId = 0x01;
    pHeader.RTR = CAN_RTR_DATA;
...
HAL_StatusTypeDef sendStatue3ViaCAN(CAN_HandleTypeDef *hcan, uint8_t FreValue[4]) {
    CAN_TxHeaderTypeDef pHeader;
    pHeader.StdId = 0x403;
    pHeader.ExtId = 0x01;
    pHeader.RTR = CAN_RTR_DATA;
...
HAL_StatusTypeDef sendStatue4ViaCAN(CAN_HandleTypeDef *hcan, uint8_t FreValue[4]) {
    CAN_TxHeaderTypeDef pHeader;
    pHeader.StdId = 0x501;
    pHeader.ExtId = 0x01;
    pHeader.RTR = CAN_RTR_DATA;
...
HAL_StatusTypeDef sendStatue5ViaCAN(CAN_HandleTypeDef *hcan, uint8_t FreValue[4]) {
    CAN_TxHeaderTypeDef pHeader;
    pHeader.StdId = 0x502;
    pHeader.ExtId = 0x01;
    pHeader.RTR = CAN_RTR_DATA;
...
HAL_StatusTypeDef sendStatue6ViaCAN(CAN_HandleTypeDef *hcan, uint8_t FreValue[4]) {
    CAN_TxHeaderTypeDef pHeader;
    pHeader.StdId = 0x503;
    pHeader.ExtId = 0x01;
    pHeader.RTR = CAN_RTR_DATA;
...
void mpu_get_data()
{
    ...
    if (can_set == 0)
    {
        // 可以选择是否继续发送加速度数据，这里假设只发送转向角，因此注释掉其他发送部分
        /*
        fu.f = mpu_data.ax;
        accel_send[0] = (fu.i >> 24) & 0xFF;
        accel_send[1] = (fu.i >> 16) & 0xFF;
        accel_send[2] = (fu.i >> 8) & 0xFF;
        accel_send[3] = fu.i & 0xFF;
        sendStatue1ViaCAN(&hcan1, accel_send);
        ...
        accel_send[2] = (fu.i >> 8) & 0xFF;
        accel_send[3] = fu.i & 0xFF;
        sendStatue2ViaCAN(&hcan1, accel_send);
        ...
        accel_send[2] = (fu.i >> 8) & 0xFF;
        accel_send[3] = fu.i & 0xFF;
        sendStatue3ViaCAN(&hcan1, accel_send);
        */
        can_set = 1;
    }
    else
    {
        ...
        accel_send[2] = (fu.i >> 8) & 0xFF;
        accel_send[3] = fu.i & 0xFF;
        sendStatue4ViaCAN(&hcan1, accel_send);
        ...
        accel_send[2] = (fu.i >> 8) & 0xFF;
        accel_send[3] = fu.i & 0xFF;
        sendStatue5ViaCAN(&hcan1, accel_send);
        */

        // 修改为发送转向角 (imu.yaw)
        fu.f = imu.yaw;  // 将转向角转换为 float
        ...
        // 通过 CAN 发送转向角
        //sendStatue6ViaCAN(&hcan1, accel_send);
        can_set = 0;
    }
}
